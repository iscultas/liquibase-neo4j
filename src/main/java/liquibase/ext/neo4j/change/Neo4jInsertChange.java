package liquibase.ext.neo4j.change;

import liquibase.change.ChangeMetaData;
import liquibase.change.ColumnConfig;
import liquibase.change.DatabaseChange;
import liquibase.change.core.InsertDataChange;
import liquibase.database.Database;
import liquibase.exception.ValidationErrors;
import liquibase.ext.neo4j.database.Neo4jDatabase;
import liquibase.ext.neo4j.statement.CypherParameterizedStatement;
import liquibase.statement.SqlStatement;

import java.util.Collections;
import java.util.HashMap;
import java.util.List;
import java.util.Map;

@DatabaseChange(name = "insert", priority = ChangeMetaData.PRIORITY_DATABASE,
        description = "Inserts data to the graph")
public class Neo4jInsertChange extends InsertDataChange {

    private String labelName = "";

    @Override
    public ValidationErrors validate(Database database) {
        ValidationErrors errors = new ValidationErrors(this);
        if (!(database instanceof Neo4jDatabase)) {
            errors.addError(String.format(
                    "unsupported database %s, expected Neo4j",
                    database == null ? "null" : database.getShortName()
            ));
        }
        if (labelName.isEmpty()) {
            errors.addError("missing label name for insert");
        }
        return errors;
    }

    public void setLabelName(String labelName) {
        this.setTableName(labelName);
    }

    public String getLabelName() {
        return labelName;
    }

    @Override
    public void setTableName(String tableName) {
        super.setTableName(tableName);
        this.labelName = tableName;
    }

    @Override
    public SqlStatement[] generateStatements(Database database) {
        return new SqlStatement[]{
                new CypherParameterizedStatement(
                        String.format("CREATE (node:`%s`) SET node += $1", this.getTableName()),
                        Collections.singletonList(propertyMap(this.getColumns()))
                )
        };
    }

    private Map<String, Object> propertyMap(List<ColumnConfig> columns) {
        Map<String, Object> result = new HashMap<>(columns.size());
        for (ColumnConfig column : columns) {
            result.put(column.getName(), column.getValueObject());
        }
        return result;
    }
}
